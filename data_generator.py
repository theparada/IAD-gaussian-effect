...
parser.add_argument('--input_dir', type=str, default="new_separated_data/",
...
if args.testset:
    np.save(os.path.join('X_files/X_test.npy'), X_test)
    np.save(os.path.join('X_files/y_test.npy'), y_test)
    print("Testset is generated.")
else:
    if args.signal_vs_bg:
        np.save(os.path.join('X_files/X_train_sb'+args.S_over_B+'.npy'), X_train)
        np.save(os.path.join('X_files/X_val_sb'+args.S_over_B+'.npy'), X_val)
        np.save(os.path.join('X_files/y_train_sb'+args.S_over_B+'.npy'), y_train)
        np.save(os.path.join('X_files/y_val_sb'+args.S_over_B+'.npy'), y_val)
        print("X_files (sb) are saved.")
    else:
        np.save(os.path.join('X_files/X_train'+args.S_over_B+'.npy'), X_train)
        np.save(os.path.join('X_files/X_val'+args.S_over_B+'.npy'), X_val)
        np.save(os.path.join('X_files/y_train'+args.S_over_B+'.npy'), y_train)
        np.save(os.path.join('X_files/y_val'+args.S_over_B+'.npy'), y_val)
